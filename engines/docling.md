...
- **The `--pipeline standard --enrich-formula` specialist path is not a viable fix.** Tested: it does avoid the token swaps (a dedicated CodeFormula model transcribes detected equation regions), but it runs ~4.7 min/page CPU-bound (≈2 h for a 24-page paper), and introduces its own structural errors (displaced subscript brackets). marker and MinerU are both faster and cleaner. Do not reach for it.

...
- **Maths is gist-grade only — never transcription-grade.** Measured (June 2026, two equation-dense papers): the 258M VLM substitutes tokens inside otherwise-valid LaTeX — a dropped leading minus sign in one paper; in the other, 6 of 25 occurrences of log(1/δ) became log(1/8) or log(1/2), with the same theorem corrupted differently in two places. The output renders cleanly, so nothing flags the error. This is a property of whole-page VLM transcription in general — a vision model reading the page natively shares it — not a Docling-specific bug. Route equation-bearing documents to marker or MinerU.
- **The `--pipeline standard --enrich-formula` specialist path is not a viable fix.** Tested: it does avoid the token swaps (a dedicated CodeFormula model transcribes detected equation regions), but it runs CPU-bound — 46 min for a 24-page paper (~4.7 min/page on equation-dense pages), and introduces its own structural errors (displaced subscript brackets). marker and MinerU are both faster and cleaner. Do not reach for it.

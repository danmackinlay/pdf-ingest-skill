...
0. **Check native ingestion first.** If the host agent reads PDFs natively (vision-capable model with a page-range PDF tool), use that for short documents and interactive Q&A — zero setup, and it reads layout, figures, and tables as rendered. Caveat: reading a page natively *is* whole-page VLM transcription, the same operation that silently corrupts maths in step 3 — so do not trust a natively-read equation, numeric cell, or exact string that will be reused verbatim; convert those with marker/MinerU (step 4) even when vision-capable. Read the page to *understand* it; convert it to *quote* it. Use this skill instead of native reading when: output *files* are required; the document is long enough that convert-once-read-selectively beats repeated native page reads; the job is batch/corpus conversion; the host model is text-only; or an answer turns on maths that must be exact.
